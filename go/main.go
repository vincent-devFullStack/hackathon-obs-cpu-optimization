...
func cosineAllPairsChecksum(ds *Dataset) float64 {
	axisNorms := make([]float64, ds.M)
	for j := 0; j < ds.M; j++ {
		base := j * ds.D
		s := 0.0
		for k := 0; k < ds.D; k++ {
...
func main() {
	...
	if *selfCheck {
		checksum := cosineAllPairsChecksum(ds)
