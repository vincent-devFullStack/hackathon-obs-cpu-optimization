...
public final class CosineBenchmarkOpt {
    ...
    private static double[] normalizeAxes(Dataset ds) {
        double[] out = new double[ds.m * ds.d];
        for (int j = 0; j < ds.m; j++) {
            ...
            for (int k = 0; k < ds.d; k++) {
                out[aBase + k] = ds.a[aBase + k] * inv;
            }
        }
        return out;
    }

    private static double cosineAllPairsChecksumOpt(Dataset ds, double[] aNormed) {
        double checksum = 0.0;
        for (int i = 0; i < ds.n; i++) {
            ...
            double invEmbNorm = 1.0 / embNorm;

            for (int j = 0; j < ds.m; j++) {
                int aBase = j * ds.d;
                double dot = 0.0;
                for (int k = 0; k < ds.d; k++) {
                    dot += ds.e[eBase + k] * aNormed[aBase + k];
                }
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < args.length; i++) {
            switch (args[i]) {
                ...
                default:
                    throw new IllegalArgumentException(
                            "Usage: java CosineBenchmarkOpt [--metadata PATH] [--warmup N] [--runs N] [--repeat N] [--expected-dataset-sha SHA] [--self-check]"
                    );
            }
        }
        ...
        double[] aNormed = normalizeAxes(ds);

        if (selfCheck) {
            double checksum = cosineAllPairsChecksumOpt(ds, aNormed);
            System.out.println(String.format(
                    Locale.US,
                    "{\"type\":\"self_check\",\"impl\":\"java-opt\",\"ok\":true,\"N\":%d,\"M\":%d,\"D\":%d,\"dataset_sha256\":\"%s\",\"checksum\":%.17g,\"runtime\":{\"available_processors\":%d,\"jvm_input_args\":%s,\"java_version\":\"%s\",\"java_vm\":\"%s\"}}",
                    ds.n,
                    ds.m,
                    ds.d,
                    ds.datasetSha256,
        ...
        System.out.println(String.format(
                Locale.US,
                "{\"type\":\"meta\",\"impl\":\"java-opt\",\"N\":%d,\"M\":%d,\"D\":%d,\"repeat\":%d,\"warmup\":%d,\"runs\":%d,\"dataset_sha256\":\"%s\",\"build_flags\":\"\",\"runtime\":%s}",
                ds.n,
                ds.m,
                ds.d,
                repeat,
        ...
        for (int i = 0; i < warmup; i++) {
            cosineAllPairsChecksumOpt(ds, aNormed);
        }
        ...
        for (int runId = 0; runId < runs; runId++) {
            ...
            String line = String.format(
                    Locale.US,
                    "{\"type\":\"run\",\"impl\":\"java-opt\",\"run_id\":%d,\"wall_ns\":%d,\"cpu_ns\":%d,\"checksum\":%.17g,\"max_rss_kb\":%d,\"ctx_voluntary\":%d,\"ctx_involuntary\":%d,\"minor_faults\":%d,\"major_faults\":%d,\"max_threads\":%d}",
                    runId,
                    wallNs,
                    cpuNs,
                    checksumAcc,
