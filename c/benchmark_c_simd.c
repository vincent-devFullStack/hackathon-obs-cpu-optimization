...
int main(int argc, char **argv) {
    ...
    if (self_check) {
        double checksum = cosine_all_pairs_checksum_simd(&ds);
        printf("{\"type\":\"self_check\",\"impl\":\"%s\",\"ok\":true,\"N\":%zu,\"M\":%zu,\"D\":%zu,\"dataset_sha256\":\"%s\",\"checksum\":%.17g}\n",
               IMPL_ID,
               ds.N,
               ds.M,
               ds.D,
    ...
    printf("{\"type\":\"meta\",\"impl\":\"%s\",\"N\":%zu,\"M\":%zu,\"D\":%zu,\"repeat\":%d,\"warmup\":%d,\"runs\":%d,\"dataset_sha256\":\"%s\",\"build_flags\":\"%s\",\"runtime\":{\"compiler\":\"%s\",\"compiler_version\":\"%s\",\"build_profile\":\"%s\",\"warmup_executed\":%d}}\n",
           IMPL_ID,
           ds.N,
           ds.M,
           ds.D,
    ...
    for (int i = 0; i < warmup; i++) {
        volatile double sink = cosine_all_pairs_checksum_simd(&ds);
        (void)sink;
    }
    ...
    for (int run_id = 0; run_id < runs; run_id++) {
        ...
        for (int r = 0; r < repeat; r++) {
            checksum_acc += cosine_all_pairs_checksum_simd(&ds);
        }
        ...
        printf("{\"type\":\"run\",\"impl\":\"%s\",\"run_id\":%d,\"wall_ns\":%lld,\"cpu_ns\":%lld,\"checksum\":%.17g,\"max_rss_kb\":%ld,\"ctx_voluntary\":%lld,\"ctx_involuntary\":%lld,\"minor_faults\":%lld,\"major_faults\":%lld,\"max_threads\":%d}\n",
               IMPL_ID,
               run_id,
               wall_ns,
               cpu_ns,
